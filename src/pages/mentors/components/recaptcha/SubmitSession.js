...
export class CreateSessionReCaptcha extends Component {
    ...
    sendConfirmEmail = async(key) => {
        ...
        const emailBody = {
            ...
            "editURL": `${window.location.origin}/profile/${this.orgId}/${this.eventId}/${this.state.sessionId}`,
            "key": key
        }
    ...
    render() {
        ...
        return (
            <span>
                <span className="float-right"> <Checkbox style={{paddingTop: '4px'}} checked={this.state.sendEmail} onChange={() => this.setState({ sendEmail : !this.state.sendEmail })}   /> I wish to have an email confirmation sent to user. 
